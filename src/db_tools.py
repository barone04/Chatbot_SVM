...
def load_llm():
    """Phải dùng litellm để load LLM cho Agent, nếu không sẽ bị báo lỗi provider"""
    llm = LLM(
        model=MODEL_NAME,
        temperature=0.1,
...
def load_llm_tools(model_name):
    """Dùng ChatGoogleGenerativeAI của Langchain để load LLM,
    bởi vì QuerySQLCheckerTool yêu cầu một BaseLanguageModel"""
...
class TablesSchemaTool(BaseTool):
    ...
    description: str = (
        ...
        "Bảng slot cho biết các sản phẩm có trong máy bán hàng, bảng bill cho biết hóa đơn mua hàng"
...
class CheckSQLTool(BaseTool):
    ...
    def _run(self, sql_query: str) -> str:
        llm = load_llm_tools("gemini-2.5-flash") #gemini-2.0-flash
